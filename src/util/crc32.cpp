...
uint32_t crc32::update(const uint8_t *p_data, size_t p_data_length)
{
   ...
   while (p_data_length--)
   {
      ...
      //Cycle through each bit of the data word, lowest to highest (This is generally for little-endianness, as defined by IEEE standard, but
      //actual endianness doesn't matter so long as we're consistent)
